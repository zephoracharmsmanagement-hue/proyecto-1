...
### Zephora Charms · las 28 respuestas, sin explicaciones

> **Este archivo es para usar. [02-macros.md](02-macros.md) es para entender** —
> ahí está por qué cada macro dice lo que dice.

## Cómo usarlo

Cada macro está dentro de un bloque de código: **lo que ves es exactamente lo
que se pega.** Selecciona todo el bloque, copia, pega en WhatsApp.

**Ojo con el formato:** estos textos usan la sintaxis de WhatsApp, no la de
Markdown. En WhatsApp la negrita es `*un asterisco*`, no dos. Si copias desde
`02-macros.md` te van a quedar los asteriscos dobles visibles en el mensaje.
Copia desde aquí.

**Antes de enviar, reemplaza los `{{marcadores}}`** por el dato real. Los que
aparecen con más frecuencia:

| Marcador | De dónde sale |
|---|---|
| `{{horario}}` | ⚠️ **Sin definir todavía** — aparece en A3 y E7 |
| `{{fin_promo}}` | ⚠️ **Sin definir todavía** — aparece en E5 |
| `{{charm.nombre}}`, `{{charm.precio}}` | `data/inventario.json` |
| `{{total}}`, `{{ciudad}}`, `{{guia}}` | El pedido concreto |

**Mantenimiento:** los precios generales van escritos directo en el texto
($58.000, $72.000, $180.000, las tarifas de envío) para que no tengas que
reemplazarlos en cada mensaje. La contra es que **si cambias precios en la web,
hay que actualizarlos aquí también.** Corre `node scripts/inventario.mjs` para
ver los valores vigentes y revisa las macros A1, B1, B5, B7, C1, C3, C5, D1 y E1,
que son las que los mencionan.
